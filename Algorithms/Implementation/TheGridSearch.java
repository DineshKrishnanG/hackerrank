...
public class TheGridSearch {
    ...
    public static void main(String[] args) {
        ...
        int tcCnt = Integer.parseInt(in.nextLine());
        while( tcCnt > 0){
            tcCnt --;
        
            // Large Grid
            String[] lDim = in.nextLine().split("\\s");
            
            int lr = Integer.parseInt(lDim[0]);
            int lc = Integer.parseInt(lDim[1]);
            char[][] lGrid = new char[lr][lc];
            for(int i=0; i< lr; i++){
                lGrid[i] = in.nextLine().toCharArray();
            }
            //sGrid
            String[] sDim = in.nextLine().split("\\s");
            int sr = Integer.parseInt(sDim[0]);
            int sc = Integer.parseInt(sDim[1]);
            char[][] sGrid = new char[sr][sc];
            for(int i=0; i< sr; i++){
                sGrid[i] = in.nextLine().toCharArray();
            }

            //Test enclosing
            String isEnclosing = "NO";
            for(int i=0; i<=lr-sr; i++){
                char[] row = lGrid[i];
                for( int j=0; j <= lc-sc; j++){
                    if( row[j] == sGrid[0][0] && 
                       isPartOf(sGrid,lGrid,i,j)){
                        isEnclosing = "YES";
                    }
                }
            }
            System.out.println(isEnclosing);
        }
    }
    
    public static boolean isPartOf(char[][] smallGrid, char[][] largeGrid, int lRow, int lColumn){
        for(int i=0; i<smallGrid.length;i++){
            char[] sRow = smallGrid[i];
            for( int j=0; j<sRow.length; j++){
                if( sRow[j] != largeGrid[i+lRow][j+lColumn] ){
                    return false;
                }
            }
        }
