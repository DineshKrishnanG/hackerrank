...
public class ServiceLane {
    ...
    public static void main(String[] args) {
        ...
        int noOfSegments = scan.nextInt();
        int noOfTestcase = scan.nextInt();
        
        int[] widtharr = new int[noOfSegments];
        for(int i=0; i <noOfSegments; i++ ){
            widtharr[i] = scan.nextInt();
        }
       
        for( int i=0; i< noOfTestcase; i++){
            //Test case start
            int sIdx = scan.nextInt();
            int eIdx = scan.nextInt();
            int limit = 0;
            for(int idx=sIdx; idx <= eIdx; idx++){
                if(limit == 0 || limit > widtharr[idx]){
                    limit = widtharr[idx];
                }
