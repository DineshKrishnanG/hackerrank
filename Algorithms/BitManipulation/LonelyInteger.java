...
public class LonelyInteger {
   static int lonelyinteger(int[] a) {
       int len = a.length;
       Map<Integer,Integer> sampler = new HashMap<Integer,Integer>();
       for( int i=0; i< len; i++){
           int num = a[i];
          if(sampler.containsKey(num)){
             sampler.remove(num); // TODO: get by object, and not by index
          }else{
              sampler.put(num,1);
          }
       }
      return sampler.keySet().iterator().next();
    }
